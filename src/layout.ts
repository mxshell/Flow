...
export function buildLayout(doc: Diagram) {
  ...
  const height = Math.max(555, Math.max(...columns.values()) * 62 + 115);
  const width = Math.max(1000, maxDepth * 220 + 340);
  const layout = sankey<NodeData, LinkData>().nodeId(n => n.name).nodeAlign(sankeyLeft).nodeWidth(doc.appearance.nodeWidth).nodePadding(32).nodeSort(null).iterations(48).extent([[155, 65], [width - 195, height - 60]]);
